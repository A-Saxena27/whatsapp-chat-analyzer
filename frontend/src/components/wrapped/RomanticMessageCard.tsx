...
export default function Card11({ data }: Props) {
  const romanticMsg = data?.mostRomanticMsg;
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-6 px-6 text-center">
      ...
      >
        <div className="text-6xl absolute -top-4 -left-2 opacity-20">
          "
        </div>

        <p className="text-white text-base leading-relaxed italic font-light relative z-10">
          {romanticMsg?.text || "No romantic messages found 💔"}
        </p>

        <div className="text-6xl absolute -bottom-8 -right-2 opacity-20">
          "
        </div>
      </Glass>

      <div className="text-white/40 text-xs font-mono">
        📅 {romanticMsg?.date || "No date available"}
      </div>
    </div>
  );
}